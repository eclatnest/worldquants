...
import sys
sys.path.append('..')
from operators_config_full import *
...
# 数据集配置
DATASET_ID = 'other543'                    # 数据集ID
REGION = 'CHN'                           # 区域: 'USA', 'EUR', 'CHN', 'GLB', 'ASI'
UNIVERSE = 'TOP2000U'                     # 宇宙: 'TOP3000', 'TOP2500', 'MINVOL1M'等
DELAY = 1                                # 延迟: 0或1
DATA_TYPE = 'MATRIX'                     # 数据类型: 'MATRIX'或'VECTOR'

# 模拟配置
NEUTRALIZATIONS = ["NONE", "REVERSION_AND_MOMENTUM", "CROWDING","FAST", "SLOW", "MARKET", "SECTOR", "INDUSTRY", "SUBINDUSTRY", "SLOW_AND_FAST"]        # 中性化（数组，可配置多个，会逐个执行）
random.shuffle(NEUTRALIZATIONS)
INIT_DECAY = 60                           # 初始衰减值
TASK_POOL_SIZE = 8                      # 任务池数量
CONCURRENT_SIMS = 1                      # 并发模拟数
...
class AlphaExpressionGenerator:
    ...
    def generate_all(self):
        ...
        print(f"  ✓ 单参数操作符: {len(PARAM_1)}个")
        
        # 2. 时间序列操作符 (双参数，需要窗口期)
        self._generate_ts_operators()
        print(f"  ✓ 时间序列操作符: {len([op for op in PARAM_2 if op.startswith('ts_')])}个")
        ...
        print(f"  ✓ 分组操作符: {len([op for op in PARAM_2 if op.startswith('group_')])}个")
    ...
    def _generate_single_param(self):
        ...
        single_ops = [op for op in PARAM_1 if not op.startswith('vec_') and op not in exclude_ops]
...
def main():
    ...
    print(f"支持操作符: {len(OPERATORS_ALL_REGULAR)}个")
    ...
    print(f"  ✓ 总字段: {len(all_fields)} | 使用: {len(fields)}")
    if fields:
        print(f"  示例: {fields[0]}")
    ...
    print(f"  预计批次: {int(len(expressions) / 65)}")
